...
warp_model = My_nequin(opt, 45)
print(warp_model)
warp_model.train()
warp_model.cuda()
load_checkpoint_parallel(warp_model, opt.MN1_warp_checkpoint)
...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    if epoch % opt.save_epoch_freq == 0:
      if opt.local_rank == 0:
        ...
        save_checkpoint(model.module, os.path.join(opt.checkpoints_dir, opt.name, 'MN1_warp_epoch_%03d.pth' % (epoch+1)))
        save_checkpoint(model_gen.module, os.path.join(opt.checkpoints_dir, opt.name, 'MN1_gen_epoch_%03d.pth' % (epoch+1)))
