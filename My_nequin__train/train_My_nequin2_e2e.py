...
from My_nequin__train.models.My_nequin import TVLoss, My_nequin
...
Mu_nequin_warp_model = My_nequin(opt, 3)
print(MN2_warp_model)
MN2_warp_model.train()
MN2_warp_model.cuda()
load_checkpoint_parallel(MN2_warp_model, opt.MN2_warp_checkpoint)

MN2_gen_model = ResUnetGenerator(7, 4, 5, ngf=64, norm_layer=nn.BatchNorm2d)
print(MN2_gen_model)
MN2_gen_model.train()
MN2_gen_model.cuda()

MN1_warp_model = My_nequin(opt, 45)
print(MN1_warp_model)
MN1_warp_model.eval()
MN1_warp_model.cuda()
load_checkpoint_parallel(MN1_warp_model, opt.MN1_warp_checkpoint)

MN1_gen_model = ResUnetGenerator(8, 4, 5, ngf=64, norm_layer=nn.BatchNorm2d)
print(MN1_gen_model)
MN1_gen_model.eval()
MN1_gen_model.cuda()
load_checkpoint_parallel(MN1_gen_model, opt.MN1_gen_checkpoint)

MN2_warp_model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(MN2_warp_model).to(device)
MN2_gen_model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(MN2_gen_model).to(device)

if opt.isTrain and len(opt.gpu_ids):
    MN2_warp_model = torch.nn.parallel.DistributedDataParallel(MN2_warp_model, device_ids=[opt.local_rank])
    MN2_gen_model = torch.nn.parallel.DistributedDataParallel(MN2_gen_model, device_ids=[opt.local_rank])
    MN1_warp_model = torch.nn.parallel.DistributedDataParallel(MN1_warp_model, device_ids=[opt.local_rank])
    MN1_gen_model = torch.nn.parallel.DistributedDataParallel(MN1_gen_model, device_ids=[opt.local_rank])

criterionL1 = nn.L1Loss()
criterionVGG = VGGLoss()
criterionL2 = nn.MSELoss('sum')

params_warp = [p for p in MN2_warp_model.parameters()]
params_gen = [p for p in MN2_gen_model.parameters()]
...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    for i, data in enumerate(train_loader):
        ...
        flow_out_un = MN1_warp_model(concat_un.cuda(), clothes_un.cuda(), pre_clothes_edge_un.cuda())
        ...
        flow_out_sup = MN1_warp_model(concat_un.cuda(), clothes.cuda(), pre_clothes_edge.cuda())
        ...
        gen_outputs_un = MN1_gen_model(gen_inputs_un)
        ...
        flow_out = MN2_warp_model(p_tryon_un.detach(), clothes.cuda(), pre_clothes_edge.cuda())
        ...
        gen_outputs = MN2_gen_model(gen_inputs)
    ...
    if epoch % opt.save_epoch_freq == 0:
        if opt.local_rank == 0:
            ...
            save_checkpoint(MN2_warp_model.module,
                            os.path.join(opt.checkpoints_dir, opt.name, 'MN2_warp_epoch_%03d.pth' % (epoch + 1)))
            save_checkpoint(MN2_gen_model.module,
                            os.path.join(opt.checkpoints_dir, opt.name, 'MN2_gen_epoch_%03d.pth' % (epoch + 1)))

    if epoch > opt.niter:
        MN2_warp_model.module.update_learning_rate_warp(optimizer_warp)
        MN2_warp_model.module.update_learning_rate(optimizer_gen)
